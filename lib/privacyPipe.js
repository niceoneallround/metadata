...
model.utils.YAML2Node = function YAML2Node(yaml, props) {
  ...
  if (yaml.obfuscation_context) {
    ...
    if (yaml.destination_provision_pipe_url) {
      if (yaml.destination_provision_pipe_url['@type']) {
        oc[PN_P.destinationProvisionPipeURL] = yaml.destination_provision_pipe_url;
      } else {
        oc[PN_P.destinationProvisionPipeURL] = PNDataModel.utils.createURLValue(yaml.destination_provision_pipe_url);
      }
    }

    if (yaml.destination_provision_basic_auth_token) {
      oc[PN_P.destinationProvisionBasicAuthToken] = yaml.destination_provision_basic_auth_token;
    }
