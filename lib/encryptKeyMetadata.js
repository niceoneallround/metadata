...
{ @id: id http://md.pn.id.webshield.io/encrypt_key/com/acme#23,
 ...
 When submitted as a JWT using the metadata claim
   ...
   pn_p.metadata:
     ...
     - pn_p.raw_encrypt_key_md_type: JSONWebKey - https://tools.ietf.org/html/rfc7517
     - pn_p.raw_encrypt_key_md: base64 encoding
...
model.utils.YAML2Node = function YAML2Node(yaml, props) {
  ...
  if (!yaml.raw_encrypt_key_md_type) {
    node[PN_P.rawEncryptKeyMDType] = 'jsonwebkey';
  } else {
    node[PN_P.rawEncryptKeyMDType] = yaml.raw_encrypt_key_md_type.toLowerCase();
  }

  // check has raw data
  if (!yaml.raw_encrypt_key_md) {
    return PNDataModel.errors.createTypeError({
      id: PNDataModel.ids.createErrorId(props.hostname, moment().unix()),
      errMsg: util.format('ERROR raw_encrypt_key_md missing from YAML format:%j', yaml),
    });
  } else {
    if ((typeof yaml.raw_encrypt_key_md === 'string') || (yaml.raw_encrypt_key_md instanceof String)) {
      node[PN_P.rawEncryptKeyMD] = yaml.raw_encrypt_key_md;
    } else {
      // assume a json object so convert to a base64
      let s = JSON.stringify(yaml.raw_encrypt_key_md);
      let b64 = Buffer.from(s).toString('base64');
      node[PN_P.rawEncryptKeyMD]  = b64;
    }
  }
...
model.utils.verify = function verify(node, props) {
  ...
  if (!node[PN_P.rawEncryptKeyMDType]) {
    return PNDataModel.errors.createTypeError({
      id: PNDataModel.ids.createErrorId(hostname, moment().unix()),
      errMsg: util.format('ERROR %s missing from:%j', PN_P.rawEncryptKeyMDType, node),
    });
  }

  if (!node[PN_P.rawEncryptKeyMD]) {
    return PNDataModel.errors.createTypeError({
      id: PNDataModel.ids.createErrorId(hostname, moment().unix()),
      errMsg: util.format('ERROR %s missing from:%j', PN_P.rawEncryptKeyMD, node),
    });
  }
...
model.canons.createTestKey = function createTestKey(props) {
  ...
  let yaml = {
    ...
    description: 'Raw is a JSON Web Key Format holding a base64 clear text key - this is a good key used in testing. As an object code will auto convert it to bas64 string',
    raw_encrypt_key_md_type: 'jsonwebkey',
    raw_encrypt_key_md: {
      kty: 'oct',
      alg: 'AES_256',
