...
model.utils.verifyTemplate = function verifyTemplate(node, props) {
  ...
  if ((node[PN_P.action] === PN_T.Obfuscate) && (node[PN_P.privacyActionInstance2Deobfuscate])) {
    return PNDataModel.errors.createTypeError({
      id: PNDataModel.ids.createErrorId(hostname, moment().unix()),
      errMsg: util.format('ERROR %s obfuscate should not have %s deobfuscate :%j', PN_P.privacyActionInstance2Deobfuscate, node),
    });
  }
