...
model.utils.create = function create(pait, paction, pp, props) {
  ...
  if (paction[PN_P.keyEncryptKeyMD]) {
    if (pait[PN_P.keyEncryptKeyMD]) {
      return PNDataModel.errors.createTypeError({
        id: PNDataModel.ids.createErrorId(props.hostname, moment().unix()),
        errMsg: util.format('ERROR PAI create - privacy action has a key encrypt key metadata cannot override. Action%j Instance:%j', paction, pait),
      });
    } else {
  ...
  if (paction[PN_P.contentEncryptKeyMD]) {
    if (pait[PN_P.contentEncryptKeyMD]) {
      return PNDataModel.errors.createTypeError({
        id: PNDataModel.ids.createErrorId(props.hostname, moment().unix()),
        errMsg: util.format('ERROR PAI create - privacy action has a content encrypt key metadata cannot override. Action%j Instance:%j', paction, pait),
      });
    } else {
  ...
  if (paction[PN_P.obfuscationService]) {
    if (pait[PN_P.obfuscationService]) {
      return PNDataModel.errors.createTypeError({
        id: PNDataModel.ids.createErrorId(props.hostname, moment().unix()),
        errMsg: util.format('ERROR PAI create - privacy action has a obfuscation service cannot override. Action%j Instance:%j', paction, pait),
      });
    } else {
  ...
  if (!node[PN_P.skipOrchestration]) {
    ...
    if (paction[PN_P.schema]) {
      if (pait[PN_P.schema]) {
        return PNDataModel.errors.createTypeError({
          id: PNDataModel.ids.createErrorId(props.hostname, moment().unix()),
          errMsg: util.format('ERROR PAI Create - privacy action has a schema cannot override. Action%j Instance:%j', paction, pait),
        });
      } else {
    ...
    if (!node[PN_P.schema]) {
      return PNDataModel.errors.createTypeError({
        id: PNDataModel.ids.createErrorId(props.hostname, moment().unix()),
        errMsg: util.format('ERROR PAI create no schema in action or instance. Action%j Instance:%j', paction, pait),
      });
    }
  }
