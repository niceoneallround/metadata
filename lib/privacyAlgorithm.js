...
model.utils.YAML2PrivacyAlgorithm = function YAML2PrivacyAlgorithm(yaml, props) {
  ...
  assert(props.domainName, util.format('props.domain missing - required to create ids:%j', props));
  console.log(yaml);
  let node = {};
...
model.utils.verifyPrivacyAlgorithm = function verifyPrivacyAlgorithm(node, hostname) {
  'use strict';
