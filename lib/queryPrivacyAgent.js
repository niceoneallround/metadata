...
It has the following properities, note the urls are part of it so can assign trust
•	@id
•	@type: Resource, QueryPrivacy Agent
•	pn_p.organization: the @id of the owning org
•	pn_p.description
•	pn_p.pndatamodel; the id of the pn data model it uses
•	pn_p.privacy_algorithm: the @id of the default pipe for data sent to the US
•	pn_p.provision_pipe_url: where to post the provision pipe info
- pn_p.post_query_result_url: where to post query results
•	pn_p.organization: the id of the owning organization
...
class QueryPrivacyAgent {
  ...
  static YAML2Node(yaml, props) {
    ...
    if (yaml.post_query_result_url) {
      node[PN_P.postQueryResultURL] = PNDataModel.model.utils.createURLValue(yaml.post_query_result_url);
    }
  ...
  static verify(node, props) {
    ...
    if (!node[PN_P.postQueryResultURL]) {
      return PNDataModel.errors.createTypeError({
        id: PNDataModel.ids.createErrorId(hostname, moment().unix()),
        errMsg: util.format('ERROR %s missing from:%j', PN_P.postQueryResultURL, node),
      });
    }
  ...
  static createTestQPA(props) {
    ...
    let yaml = {
      ...
      post_query_result_url: 'http://query.fake.test.webshield.io/query_result',
