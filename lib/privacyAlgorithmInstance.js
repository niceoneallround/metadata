...
@param palgit - the privacy algorithm instance template passed in the privacy pipe
@param palg- the privacy algorithm
@param pp - the privacy pipe the instance will run in used just for id, can be id of object
@param props.hostname
@param props.domainName

@return the privacy algorithm instance node
*/
model.utils.create = function create(palgit, palg, pp, props) {
  'use strict';
  assert((arguments.length === 4), util.format('create neeeds 4 arguments got:%s', arguments.length));
  ...
  node[PN_P.privacyPipe] = JSONLDUtils.getId(pp);
